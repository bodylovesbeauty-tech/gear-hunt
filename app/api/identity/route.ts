...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from('bbbt_identities').select('id,application_id,full_name,handle,mobile,email,requested_role,status,payload,created_at').or(`email.eq.${login.toLowerCase()},mobile.eq.${login},handle.eq.${login.replace(/^@/, '').toLowerCase()}`).maybeSingle()
