...
export async function GET(request: Request) {
  ...
  const groups = await Promise.all((data || []).map(async row => {
    ...
    return publicGroup(row, members.count || 0)
  }))
...
export async function POST(request: Request) {
  try {
    ...
    const { error: membershipError } = await supabase.from('bbbt_group_memberships').upsert({ id: `GM-${group.id}-${identity.id}`, group_id: group.id, user_id: identity.id, role: 'Group Admin', status: 'ACTIVE' }, { onConflict: 'group_id,user_id' })
