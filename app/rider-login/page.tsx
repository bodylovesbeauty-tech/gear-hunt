...
export default function RiderLoginPage() {
  ...
  async function signInWithGoogle() { const supabase = createClient(); setBusy(true); setMessage(''); const callback = new URL(process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL ?? `${window.location.origin}/auth/callback`); if (searchParams.get('next') === '/admin') callback.searchParams.set('next', '/admin'); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: callback.toString() } }); if (error) { setBusy(false); setMessage(error.message.toLowerCase().includes('provider') ? 'Google login is not configured yet.' : 'Google login could not be started.'); } }
