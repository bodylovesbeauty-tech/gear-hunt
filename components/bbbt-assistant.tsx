...
export function BBBTAssistant({ compact = false }: { compact?: boolean }) {
  ...
  const respond = (text: string) => {
    ...
    setAnswer(responses[intent]); setMessage(''); setFeedback(null); telemetry(intent === 'UNKNOWN' ? 'VOICE_INTENT_FAILED' : 'VOICE_INTENT_DETECTED', { intent, language: voiceLanguage }); if (intent === 'OPEN_PROFILE') window.location.href = '/dashboard/profile'; if (intent === 'OPEN_SAFETY_KIT') window.location.href = '/dashboard/safety-kit'; if (intent === 'OPEN_RIDES') window.location.href = '/dashboard/rides'
  }
