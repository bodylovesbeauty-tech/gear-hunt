...
export function BbbtV2Home() {
  ...
  return (
    <main className="v2-home">
      ...
      <section className="v2-protocol v2-section"><div className="v2-protocol-head"><p className="v2-kicker">THE ARCHITECTURE OF TRUST</p><h2>Trust <span>Protocol.</span></h2><p className="v2-intro-copy">A system map, not a feature list. Select a layer to understand what it is intended to do.</p><p className="v2-protocol-meta">10 SYSTEM LAYERS · ONE TRUST FRAMEWORK</p></div><div className="v2-arch"><div className="v2-core" aria-label="BBBT Trust Core">BBBT<strong>TRUST CORE</strong><small>SAFETY · TRUST<br />TRANSPARENCY · UNITY</small></div><div className="v2-arch-map">{protocol.map(([num, title]) => <button className={`v2-station ${openProtocol === Number(num) - 1 ? 'active' : ''}`} key={num} onClick={() => setOpenProtocol(Number(num) - 1)} aria-pressed={openProtocol === Number(num) - 1}><span className="v2-station-num">{num}<i>LAYER</i></span><strong className="v2-station-title">{title}</strong><small className="v2-station-status">{openProtocol === Number(num) - 1 ? 'ACTIVE' : 'CONCEPT'}</small></button>)}</div></div><div className="v2-protocol-detail" aria-live="polite"><div className="v2-detail-head"><span className="v2-detail-index">{protocol[openProtocol][0]}</span><span className="v2-detail-tag">SELECTED LAYER / 10</span></div><b>{protocol[openProtocol][1]}</b><p><strong>WHAT IT DOES</strong>{protocol[openProtocol][2]}</p><p><strong>WHY IT MATTERS</strong>Keeps the safety concept accountable, reviewable, and grounded in rider needs.</p><small>STATUS: PROTOTYPE / VERIFIED SOURCE REQUIRED</small></div></section>
