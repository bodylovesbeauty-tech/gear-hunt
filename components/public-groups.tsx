...
export function PublicGroupDetail({ identifier }: { identifier: string }) {
  ...
  const joinGroup = () => {
    ...
    if (!identity) { window.location.href = `/signup?role=Rider&group=${encodeURIComponent(group.identifier)}&returnTo=${encodeURIComponent(`/community/groups/${group.identifier}`)}`; return }
    if (stored?.id === group.targetId) {
      const next = syncLegacyGroupMembership(stored, identity)
      sessionStorage.setItem(groupKey, JSON.stringify(next))
      setNotice(next.members.some(member => member.id === identity.id) ? 'Already a member or membership saved.' : 'Membership saved to shared persistence.')
    }
    fetch('/api/network',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'join-group',userId:identity.id,groupId:group.targetId})}).then(response => { if (!response.ok) throw new Error('Unable to join group'); setNotice('Membership saved to shared persistence.') }).catch(() => setNotice('Unable to save membership. Please try again.'))
  }
