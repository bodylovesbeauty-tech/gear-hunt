...
export function PublicGroupDetail({ identifier }: { identifier: string }) {
  ...
  const joinGroup = () => {
    ...
    fetch('/api/network',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'join-group',userId:identity.id,groupId:group.targetId})}).catch(()=>{})
