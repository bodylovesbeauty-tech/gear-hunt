...
export function UnifiedSignup(){
  ...
  const signupStepKeys=['identity',...(showVehicleStep(role)?['vehicle']:[]),'location',...(showVehicleStep(role)?['blood','bloodReport','emergencyContacts','responsibilities','safetySpending']:[]),...(role==='Rider'?[]:['branch']),...(isInvestor||role==='Rider'||role==='Group Marshal'||role==='Independent Marshal'||role==='Founding Rider Council Member'?[]:['safety']),'role','review','submit'] as const
  ...
  useEffect(()=>{const params=new URLSearchParams(window.location.search);const returnTo=params.get('returnTo');if(returnTo&&returnTo.startsWith('/'))sessionStorage.setItem(returnContextKey,JSON.stringify({path:returnTo,referralToken:params.get('ref')||undefined,createdAt:new Date().toISOString()}))},[])
  const [checked,setChecked]=useState(false)
  const [showResp,setShowResp]=useState(false)
  const [step,setStep]=useState(0)
  const currentStep=signupStepKeys[step]
  const roleStepIndex=(selectedRole:Role)=>{const keys=['identity',...(showVehicleStep(selectedRole)?['vehicle']:[]),'location',...(showVehicleStep(selectedRole)?['blood','bloodReport','emergencyContacts','responsibilities','safetySpending']:[]),...(selectedRole==='Rider'?[]:['branch']),...(selectedRole==='Investor'||selectedRole==='Rider'||selectedRole==='Group Marshal'||selectedRole==='Independent Marshal'||selectedRole==='Founding Rider Council Member'?[]:['safety']),'role','review'];return keys.indexOf(selectedRole==='Rider'?'safetySpending':'role')}
  ...
  function validateStep(i:number){
    ...
    if(currentStep==='vehicle'&&showVehicleStep(role)){const registrations=vehicles.map(v=>normalizeRegistration(v.registration)).filter(Boolean);if(new Set(registrations).size!==registrations.length)e.vehicleRegistration='This registration number is already used for another vehicle in this application.';if(role==='Rider'){const primary=vehicles[0];if(!primary?.fullBikePhoto)e.fullBikePhoto='Upload a full vehicle image with the registration plate visible';if(!primary?.meterPhoto)e.meterPhoto='Upload a meter / instrument console image'}}
    ...
    if(currentStep==='review'&&role==='Rider'){
      if(!vehicles[0]?.fullBikePhoto)e.fullBikePhoto='Upload the full vehicle image before submitting'
      if(!vehicles[0]?.meterPhoto)e.meterPhoto='Upload the meter / console image before submitting'
  ...
  function submit(ev:React.FormEvent){
    ...
    fetch('/api/identity',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'sync-identity',identity})}).catch(()=>{})
    if(role==='Group Admin'){
...
<label className="su-auto-field">DISTRICT (Auto-detected)<input value={f.district} readOnly aria-readonly="true" placeholder="Detected from PIN"/></label>
  <label className="su-auto-field">STATE / UNION TERRITORY (Auto-detected)<input value={f.state} readOnly aria-readonly="true" placeholder="Detected from PIN"/></label>
      ...
      {currentStep==='branch'&&<div className="su-step signup-branch-step" data-branch={activeSignupBranch.key}>
        ...
        {role==='Group Admin'?<fieldset className="form-section"><legend>GROUP ADMIN INFORMATION</legend><label className={errors.groupName?'invalid':''}>GROUP / COMMUNITY NAME <span className="field-hint">Required</span><input value={f.groupName} onChange={set('groupName')} placeholder="BBBT QA GROUP — DO NOT RETAIN" aria-required="true"/>{errors.groupName&&<span className="field-error" role="alert">{errors.groupName}</span>}</label><label className={errors.groupProfile?'invalid':''}>PUBLIC COMMUNITY PROFILE <span className="field-hint">Required</span><textarea rows={4} value={f.groupProfile} onChange={set('groupProfile')} placeholder="Public description or community profile URL" aria-required="true"/>{errors.groupProfile&&<span className="field-error" role="alert">{errors.groupProfile}</span>}</label><div className="form-grid"><label>COMMUNITY SIZE <span className="field-hint">Optional</span><input value={f.groupSize} onChange={set('groupSize')} inputMode="numeric" placeholder="Approximate riders"/></label><label>COMMUNITY HANDLE <span className="field-hint">Optional</span><input value={f.groupHandle} onChange={set('groupHandle')} placeholder="@bbbt-qa-group"/></label></div></fieldset>:role==='Group Marshal'?<fieldset className="form-section"><legend>GROUP MARSHAL ONBOARDING</legend><div className="su-note"><strong>PRODUCT DECISION REQUIRED</strong><br/>No additional Group Marshal fields are defined in the approved prototype model. Submit your canonical identity and role request without inventing group ownership, financial, or private rider requirements.</div><p className="auth-lede" style={{margin:0}}>Group Marshal access is scoped to applicable group and ride responsibilities after review. It does not grant Group Admin ownership controls.</p></fieldset>:<div className="signup-branch-panel"><strong>{role}</strong><span>{role==='Investor'?'INVESTOR ONBOARDING — PROTOTYPE':role==='Independent Marshal'?'INDEPENDENT MARSHAL ONBOARDING — PROTOTYPE':role}</span><small>{role==='Investor'?'Investor-specific product requirements are pending product decision. No Rider safety or medical fields are collected.':'Role-specific fields are pending product decision. No additional requirements are invented in this prototype.'}</small></div>}
      </div>}
      ...
      {currentStep==='review'&&<div className="su-step su-review">
        <div className="su-review-grid">
          ...
          {role==='Group Marshal'&&<div className="su-review-section-title"><strong>GROUP MARSHAL ONBOARDING</strong><button type="button" className="text-button" onClick={()=>editStep('branch')}>EDIT</button></div>}
          {role==='Group Marshal'&&<div><small>Role-specific information</small><p>PRODUCT DECISION REQUIRED — no additional fields collected in this prototype.</p></div>}
          {role==='Group Admin'&&<><div><small>Group / community name</small><p>{f.groupName.trim()||'—'}</p></div><div><small>Public community profile</small><p>{f.groupProfile.trim()||'—'}</p></div><div><small>Community size</small><p>{f.groupSize.trim()||'Not provided'}</p></div><div><small>Community handle</small><p>{f.groupHandle.trim()||'Not provided'}</p></div></>}{role==='Founding Rider Council Member'&&<><div className="su-review-section-title"><strong>FOUNDING RIDER COUNCIL INFORMATION</strong><button type="button" className="text-button" onClick={()=>editStep('role')}>EDIT</button></div><div><small>Council email</small><p>{f.councilEmail.trim()||'—'}</p></div><div><small>Public profile</small><p>{f.councilProfile.trim()||'—'}</p></div><div><small>Years riding</small><p>{f.councilYears.trim()||'Not provided'}</p></div><div><small>Organised rides</small><p>{f.councilRides.trim()||'Not provided'}</p></div><div><small>Contribution</small><p>{f.councilWhy.trim()||'Not provided'}</p></div><div><small>Conflict disclosure</small><p>{f.councilConflict.trim()||'Not provided'}</p></div></>}
          {role==='Rider'&&<>
      ...
      <div className="su-nav">
        ...
        {step<steps.length-1
          ...
          : <button className="btn btn-cyan" type="submit" disabled={role==='Rider'&&!checked} onClick={()=>{if(role==='Rider'&&!checked)setErrors({consent:'Please acknowledge your role responsibilities to continue.'})}}>SUBMIT APPLICATION <ArrowRight size={16}/></button>}
      </div>
    </form>
