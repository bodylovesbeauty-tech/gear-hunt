...
export type PrototypeGroup={id:string;name:string;createdAt:string;cycleStart?:string;cycleEnd?:string;adminId:string;members:GovernanceMember[];elections:GovernanceElection[];rides:GovernanceRide[];notifications:GovernanceNotification[];shareToken?:string;image?:{name:string;dataUrl:string}|null;status?:'ACTIVE'|'SUSPENDED'|'REASSIGNMENT_REQUIRED';marshalIds?:string[];complaints?:ComplaintRecord[];audit?:GovernanceAudit[];scope?:GovernanceScope;permanent?:true;capabilities?:GovernanceCapability[]}
...
export type PrototypeIdentity={id:string;applicationId:string;fullName:string;handle:string;mobile:string;email:string;requestedRole:Role;status:Status;selectedLanguages:string[];vehicles:PrototypeVehicle[];safetyKit?:PrototypeSafetyKit;address?:string;city?:string;state?:string;district?:string;pinCode?:string;bbbtZone?:string;emergencyName?:string;emergencyNumber?:string;bloodGroup?:string;profilePhoto?:{name:string;dataUrl:string}|null;bloodReport?:{name:string;dataUrl:string;reportDate:string;uploadedAt:string;status:'REPORT UPLOADED'}|null;createdAt:string}
